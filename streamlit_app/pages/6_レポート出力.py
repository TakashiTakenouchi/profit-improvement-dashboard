...
from utils.optimization import get_monthly_comparison, get_mlmip_report_section
...
def generate_summary_report(params, metrics, logistic_results=None, mlmip_details=None):
    """分析サマリーレポートを生成（ケース別コメント付き、v3.0: ML-MIP詳細対応）"""
    ...
    # 最適化モード判定
    optimization_mode = params.get('use_mlmip', False)
    mode_str = "ML-MIP統合最適化（v3.0）" if optimization_mode else "要因ベース最適化（v2.0）"
...
def main():
    ...
    if report_type == "📄 最適化結果レポート":
        ...
        if not has_optimized:
            ...
            mlmip_details = st.session_state.get('mlmip_details')  # v3.0追加

            # レポート生成（v3.0: ML-MIP詳細対応）
            report = generate_summary_report(params, metrics, logistic_results, mlmip_details)
